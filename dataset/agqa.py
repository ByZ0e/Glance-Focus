...
class VideoQADataset(Dataset):
    ...
    def make_datalist(self):
        raw_datalist = load_json(self.anno_path)
        print(f"Loaded data size {len(raw_datalist)}")

        datalist = []
        qid = 0
        for question_id, anno in raw_datalist.items():
            # pdb.set_trace()
            d = dict(
            ...
            qid += 1
            d.update({key: anno[key] for key in ['global', 'ans_type', 'semantic', 'structural']})
            datalist.append(d)
